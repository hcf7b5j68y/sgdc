...
import ch.javasoft.bitset.search.CheckedBitSet;
...
//	}
	...
	public Collection<CheckedBitSet> retransform(Set<CheckedBitSet> bitsets) {
		return bitsets.stream()
				.map(checkedBitSet -> {
					IBitSet transformed = bitsetRetransform(checkedBitSet.getBitset());
					return new CheckedBitSet(transformed, checkedBitSet.isChecked()); // 假设CheckedBitSet有isChecked()方法
				})
